 ...
 *
 * Notes:
 * - Thumbs are set via JS settings on Uiza init, not HTML5 'track' property. Using the track property would be a bit gross, because it doesn't support custom 'kinds'. kind=metadata might be used for something else, and we want to allow multiple thumbnails tracks. Tracks must have a unique combination of 'kind' and 'label'. We would have to do something like kind=metadata,label=thumbnails1 / kind=metadata,label=thumbnails2. Square peg, round hole
...
class PreviewThumbnails {
  ...
  constructor(player) {
    ...
    // Only load after init all elements
    setTimeout(() => {
      this.load();
    }, 100);
  }
  ...
  endScrubbing() {
    ...
    if (Math.ceil(this.lastTime) === Math.ceil(this.player.media.currentTime)) {
      ...
      once.call(this.player, this.player.media, events.TIME_UPDATE, () => {
  ...
  /**
   * Setup hooks for Uiza and window events
   */
  listeners() {
   ...
   */
  render() {
    // Create HTML element: uiza__preview-thumbnail-container
    ...
    // Create HTML element: uiza__preview-scrubbing-container
  ...
  setThumbContainerPos() {
    ...
    const uizaRect = this.player.elements.container.getBoundingClientRect();
    const { container } = this.elements.thumb;
    // Find the lowest and highest desired left-position, so we don't slide out the side of the video container
    const minVal = uizaRect.left - seekbarRect.left + 10;
    const maxVal = uizaRect.right - seekbarRect.left - container.clientWidth - 10;
