...
const controls = {
  ...
  createLive() {
    const liveStream = createElement(
      'div',
      {
        class: `${this.config.classNames.live}`,
        'aria-label': 'live',
      },
  ...
  createContextMenu() {
    const contextMenu = createElement('div', { class: `${this.config.classNames.contextmenu}` });
    // Loop through contextmenu in order
    this.config.contextmenus.forEach(menu => {
      const element = createElement('div', {}, i18n.get(menu, this.config));
      ...
      on(element, 'click', () => {
        ...
        if (this.contextmenuShow) {
          controls.toggleContextMenu.call(this, 'hide');
        }
      });
      if (this.config.sharing && (menu === 'embed' || menu === 'sharelink')) {
  ...
  updateTimeDisplay(target = null, time = 0, inverted = false) {
    ...
    if (this.isLive) {
      // time = totalDuration - currentTime <= 10 multiply targetDuration
      if (this.uiza.targetduration && time <= this.uiza.targetduration * 10) {
        toggleClass(this.elements.live, 'live', true);
      } else {
    ...
    if (event) {
      switch (event.type) {
        ...
        case events.SEEKED:
          ...
          if (event.type === events.TIME_UPDATE) {
  ...
  updateSeekTooltip(event) {
    ...
    controls.updateTimeDisplay.call(
      ...
      (this.duration / 100) * (this.isLive ? 100 - percent : percent),
    );
  ...
  timeUpdate(event) {
    ...
    const invert = this.isLive || (!is.element(this.elements.display.duration) && this.config.invertTime);
    ...
    if (event && event.type === events.TIME_UPDATE && this.media.seeking) {
      return;
    }
    // Playing progress
  ...
  durationUpdate() {
    ...
    if (!this.isLive && !hasDuration && this.config.displayDuration && this.paused) {
      controls.updateTimeDisplay.call(this, this.elements.display.currentTime, this.duration);
    }

    // If there's a duration element, update content
    if (!this.isLive && hasDuration) {
  ...
  toggleContextMenu(command) {
    this.elements.contextmenu.style.display = command === 'show' ? 'block' : 'none';
    this.contextmenuShow = !this.contextmenuShow;
  },

  // Set postion and show context menu
  setContextMenu(position) {
    const { left, top } = position;
    this.elements.contextmenu.style.left = `${left}px`;
    this.elements.contextmenu.style.top = `${top}px`;
    controls.toggleContextMenu.call(this, 'show');
  },
  ...
  toggleMenu(input) {
    ...
    if (is.boolean(input)) {
      ...
      // If Uiza is in a shadowDOM, the event target is set to the component, instead of the
  ...
  showMenuPanel(type = '', tabFocus = false) {
    const target = this.elements.container.querySelector(`#uiza-settings-${this.id}-${type}`);
    // Limit height for scrolling
    const menu = target.querySelector('[role="menu"]');
    const maxHeight = this.elements.container.clientHeight - 2 * this.elements.controls.clientHeight;
  ...
  create(data) {
    ...
    this.elements.container.appendChild(this.elements.watching);
    this.elements.container.appendChild(this.elements.live);
    this.elements.container.appendChild(this.elements.stats);
    ...
    const defaultAttributes = { class: 'uiza__controls__item' };

    // Loop through controls in order
    dedupe(this.config.controls).forEach(control => {
      // Restart button
      if (control === 'restart' && this.config.ui.restart) {
        container.appendChild(createButton.call(this, 'restart', defaultAttributes));
      }

      // Rewind button
      if (control === 'rewind' && this.config.ui.rewind) {
        container.appendChild(createButton.call(this, 'rewind', defaultAttributes));
      }

      // Play/Pause button
      if (control === 'play' && this.config.ui.play) {
        container.appendChild(createButton.call(this, 'play', defaultAttributes));
      }

      // Fast forward button
      if (control === 'fast-forward' && this.config.ui.fastForward) {
      ...
      if (control === 'progress') {
        const progressContainer = createElement('div', {
          class: `${defaultAttributes.class} uiza__progress__container`,
        });
        if (this.config.ui.progress) {
          const progress = createElement('div', getAttributesFromSelector(this.config.selectors.progress));

          // Seek range slider
          progress.appendChild(
            createRange.call(this, 'seek', {
              id: `uiza-seek-${data.id}`,
            }),
          );
      ...
      if (control === 'current-time' && this.config.ui.currentTime) {
        container.appendChild(createTime.call(this, 'currentTime', defaultAttributes));
      }
      // Media duration display
      if (control === 'duration' && this.config.ui.duration) {
      ...
      if (control === 'mute' || control === 'volume') {
        ...
        if (!is.element(volume) || !container.contains(volume)) {
          volume = createElement(
            ...
            extend({}, defaultAttributes, {
              class: `${defaultAttributes.class} uiza__volume`.trim(),
            }),
          );
        ...
        // Toggle mute button
        if (control === 'mute' && this.config.ui.mute) {
          volume.appendChild(createButton.call(this, 'mute'));
        }

        // Volume range control
        if (control === 'volume' && this.config.ui.volume) {
          // Set the attributes
          const attributes = {
            max: 1,
            step: 0.05,
      ...
      if (control === 'captions' && this.config.ui.captions) {
        container.appendChild(createButton.call(this, 'captions', defaultAttributes));
      }

      // Settings button / menu
      if (control === 'settings' && !is.empty(this.config.settings) && this.config.ui.settings) {
        ...
        wrapper.appendChild(
          createButton.call(this, 'settings', {
            'aria-haspopup': true,
            'aria-controls': `uiza-settings-${data.id}`,
            'aria-expanded': false,
          }),
        );

        const popup = createElement('div', {
          class: 'uiza__menu__container',
          id: `uiza-settings-${data.id}`,
          hidden: '',
        });
      ...
      if (control === 'pip' && support.pip && this.config.ui.pip) {
        container.appendChild(createButton.call(this, 'pip', defaultAttributes));
      }

      // Toggle fullscreen button
      if (control === 'fullscreen' && this.config.ui.fullscreen) {
  ...
  inject() {
    ...
    if (this.config.loadSprite) {
      ...
      if (icon.cors) {
        loadSprite(icon.url, 'sprite-uiza');
      }
    }
